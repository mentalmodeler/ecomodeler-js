...
function mmpXMLToJSON(xmlString, excludeArray = []) {
    let json = {};
...
function getJSONFromNode(xmlNode, omitLocalNameProperty) {
    ...
    xmlNode.childNodes.forEach((node) => {
        if (node.nodeType === 1) {
            if (node.localName === 'influence') {
                // node.textContent = updateInfluenceValue(node.textContent);
                console.log("update influence value... ???");
            }
